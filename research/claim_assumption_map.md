...
| Results generalise beyond the original sample | Similar performance after December 2013 | No structural break, no factor decay, stable portfolio construction | True out-of-sample extension and rolling tests |
| The factor represents monetary-policy risk | Identified policy shocks retain the pricing result | Rate innovation is not dominated by central-bank information or endogenous macro responses | High-frequency policy and information decomposition |

## Mandatory distinction

The repository must distinguish among the following statements.

1. **Association** means portfolio returns covary with a rate innovation.
2. **Pricing** means the cross-sectional covariance pattern is associated with average returns under an asset-pricing model.
3. **Economic mechanism** means the factor maps to a theoretically specified state variable.
4. **Causal monetary-policy effect** requires separate identification.

No result may be promoted from one level to the next without additional evidence.
