...
| `federal_funds_rate` | `article_source_located_documented_reconstruction_series_declared` | `false` | `data/raw/fred/federal_funds_rate.csv` | `verify_before_redistribution` | Article identifies St. Louis Federal Reserve Bank federal funds rate; FEDFUNDS is the documented reconstruction series but download vintage and transformation audit remain unfrozen |
| `treasury_bill_rate` | `article_source_located_documented_reconstruction_series_declared` | `false` | `data/raw/rates/treasury_bill_rate.csv` | `verify_before_redistribution` | Article identifies St. Louis Federal Reserve Bank 3-month Treasury-bill rate; TB3MS is the documented reconstruction series and DTB3 is sensitivity-only until an aggregation rule is verified |
