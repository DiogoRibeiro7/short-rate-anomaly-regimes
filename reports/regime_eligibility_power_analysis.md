...
Date: 2026-08-06.
Generated by `scripts/analyse_regime_power.py`.

## 0. Disclosure, before anything else

**This analysis was produced AFTER the frozen 36-month and 60-month eligibility
...
**No threshold has been changed.** `configs/regimes.yaml`, every contract file in
`research/`, and `research/regime_registry.csv` are untouched. The values 36, 60,
and 10 appear in this analysis only as reference marks drawn on a curve; the
script never evaluates them as gates and never writes them.

**Acting on this evidence would be a post-hoc design change.** A threshold
revised after seeing that it binds is not the same object as a threshold frozen
before estimation, whatever the revision. If the owner chooses to revise, that
revision needs its own justification, its own entry in
`reports/design_correction_changelog.md`, and a disclosure that it was made with
the binding outcome already known. This report supplies evidence and does not
recommend a number.
...
| Months | 12 | 24 | **36** | 48 | 51 | **60** | 84 | 120 | 180 | 240 | 444 | 648 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| worst SD | 0.562 | 0.353 | 0.288 | 0.255 | 0.250 | 0.233 | 0.214 | 0.191 | 0.173 | 0.168 | 0.151 | 0.139 |
| worst RMSE | 0.583 | 0.416 | 0.360 | 0.341 | 0.331 | 0.313 | 0.288 | 0.274 | 0.246 | 0.235 | 0.197 | 0.177 |
...
**The 71-month entry is a hard computational fact, not a judgement.** The
design's own `residual_covariance_from_first_pass` refuses to build a covariance
from no more months than assets, so on the confirmatory 70-portfolio system the
Shanken covariance, the t-statistics, and the chi-square specification test do
not exist below 71 months. Both regimes that currently clear the frozen 60-month
floor are longer than that (`elb_qe` 84, `conventional_pre_elb` 444), so no
registered regime is affected today. It is a latent gap in the tier definition,
recorded as an observation: a regime of 60 to 70 months would satisfy the frozen
`months >= 60 and test_assets >= 10 and beta_rank == K` tier while the standalone
second pass it authorises could not be computed on the confirmatory asset set.
...
## 5. What the evidence says about 36 and 60

**Neither floor is too strict. On this evidence both are, if anything, too lax.**

Every criterion on the sweep except one is met at a window far longer than 60
months, and the one met at 60 is met for the wrong reason. Sign reliability
arrives at 444 months. Half of the true risk price is recovered at 444 months.
Total fitted-premium error falls under the project's own 0.25 bound at 180
months. Nominal 95 percent Shanken coverage for `lambda_rate` is not reached
anywhere on a sweep that runs to 648 months.

At the 36-month first-pass floor the estimator returns 11 percent of the true
risk price and gets its sign wrong 35 percent of the time. At the 60-month
standalone-second-pass floor it returns 14 percent and gets the sign wrong 30
percent of the time, with a 2.2 percent chance of a correctly signed significant
result.
...
restrictive side. The most defensible action on this evidence is to leave the
floors exactly as frozen and cite this analysis as a stated limitation on the
precision available inside the regimes that do clear them.
